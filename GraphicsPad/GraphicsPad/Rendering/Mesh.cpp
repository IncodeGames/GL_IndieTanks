...
void IndexedModel::AddVertex(const glm::vec3& vert)
{
	m_positions.push_back(vert);
}

void IndexedModel::AddTexCoord(const glm::vec2& texCoord)
{
	m_texCoords.push_back(texCoord);
}

void IndexedModel::AddNormal(const glm::vec3& normal)
{
	m_normals.push_back(normal);
}

void IndexedModel::AddTangent(const glm::vec3& tangent)
{
...
void IndexedModel::CalcNormals()
{
	...
	for (unsigned int i = 0; i < m_indices.size(); i += 3)
	{
		...
		glm::vec3 normal = glm::cross(v1, glm::normalize(v2)); //TODO: normalize;
